...
export async function runNewsDigest(): Promise<{ total: number; sports: number }> {
  ...
  for (const sport of sports) {
    try {
      ...
      log.info(
        { sport: sport.name, count: rawArticles.length },
        `Fetched raw articles for ${sport.name}`,
      )
      ...
      log.info(
        { sport: sport.name, count: hotTopics.length },
        `Detected hot topics for ${sport.name} (score >= 3)`,
      )
