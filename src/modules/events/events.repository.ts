...
import type { Event, NewEvent, ListEventsParams, EventWithRelations } from './events.types.js'
...
export const eventsRepository = {
  async findMany(params: ListEventsParams): Promise<Event[]> {
    ...
    if (params.cursor) {
      ...
      conditions.push(
        lt(events.created_at, sql`(SELECT created_at FROM events WHERE id = ${decodedId})`),
      )
    }
  ...
  async findFinishedWithoutArticle(limit = 50): Promise<Event[]> {
    return db
      ...
      .where(and(eq(events.status, 'finished'), isNull(articles.id)))
