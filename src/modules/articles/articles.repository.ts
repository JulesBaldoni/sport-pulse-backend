...
export const articlesRepository = {
  async findMany(params: ListArticlesParams): Promise<Article[]> {
    const conditions = [eq(articles.status, 'published'), isNull(articles.deleted_at)]
    ...
    if (params.cursor) {
      ...
      conditions.push(
        lt(articles.created_at, sql`(SELECT created_at FROM articles WHERE id = ${decodedId})`),
      )
    }
  ...
  async updateStatus(id: string, status: 'generating' | 'published' | 'failed'): Promise<Article> {
    const result = await db.update(articles).set({ status }).where(eq(articles.id, id)).returning()
    return result[0]!
  },
