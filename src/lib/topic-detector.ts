...
export function extractEntities(titles: string[]): string[] {
  ...
  for (const title of titles) {
    ...
    for (const word of words) {
      ...
      if (clean.length > 2 && /^[A-ZÀÂÄÉÈÊËÎÏÔÙÛÜ]/.test(clean) && !STOP_WORDS_UPPER.has(clean)) {
...
export function detectHotTopics(articles: RawArticle[], now: Date = new Date()): DetectedTopic[] {
