...
class GuardianClient {
  ...
  async getLatestBySport(sportSlug: string): Promise<GuardianArticle[]> {
    ...
    try {
      ...
      return this.mapResults(data.response?.results ?? []).filter((a) => a.publishedAt >= cutoff)
    } catch (err) {
      log.warn(
        { sportSlug, error: err instanceof Error ? err.message : String(err) },
        'Guardian getLatestBySport failed',
      )
      return []
    }
  }
