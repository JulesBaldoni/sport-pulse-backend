...
function makeRawFixture(id = 1): RawFixture {
  return {
    fixture: { id, date: '2026-03-10T20:00:00Z', status: { short: 'FT' }, venue: { name: 'Stade' } },
    league: { name: 'Ligue 1' },
    teams: {
...
const mockSport = { id: 'sport-uuid-001', name: 'Football', slug: 'football', created_at: new Date() };
const mockTeamHome = { id: 'team-home-001', external_id: '10', name: 'PSG', short_name: null, logo_url: null, sport_id: 'sport-uuid-001', country: null, created_at: new Date() };
const mockTeamAway = { id: 'team-away-001', external_id: '11', name: 'OM', short_name: null, logo_url: null, sport_id: 'sport-uuid-001', country: null, created_at: new Date() };
...
describe('runEventsSync', () => {
  ...
  it('throws when sportsApiClient throws (cron wrapper catches it)', async () => {
    mockSportsApiClient.getRecentFinishedFixtures.mockRejectedValue(
