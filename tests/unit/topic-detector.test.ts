...
function makeTopic(overrides: Partial<DetectedTopic> = {}): DetectedTopic {
  return {
    ...
    sources: [makeArticle({ source: "L'Equipe" }), makeArticle({ source: 'RMC Sport' })],
    sourcesCount: 2,
    score: 0,
    ...overrides,
  }
...
describe('detectHotTopics', () => {
  ...
  it('sorts by score DESC', () => {
    ...
    const articles = [
      ...
      makeArticle({
        title: 'Mbappé signs deal',
        source: 'BBC',
        url: 'u1',
        publishedAt: new Date(now.getTime() - 10 * 60 * 1000),
      }),
      makeArticle({
        title: 'Mbappé new contract confirmed',
        source: 'RMC',
        url: 'u2',
        publishedAt: new Date(now.getTime() - 20 * 60 * 1000),
      }),
      makeArticle({
        title: 'Mbappé deal done',
        source: "L'Equipe",
        url: 'u3',
      ...
      makeArticle({
        title: 'Ronaldo scores again',
        source: 'Sky',
        url: 'u4',
        publishedAt: new Date(now.getTime() - 3 * 60 * 60 * 1000),
      }),
      makeArticle({
        title: 'Ronaldo hat-trick',
        source: 'ESPN',
        url: 'u5',
        publishedAt: new Date(now.getTime() - 3.5 * 60 * 60 * 1000),
      }),
    ]
