...
describe.runIf(enabled)('GET /api/articles/search', () => {
  ...
  it('returns matching articles via full-text search', async () => {
    ...
    await seed.article({
      ...
      content:
        'Le transfert de la décennie vient de se confirmer officiellement par les deux clubs.',
    })
    await seed.article({
      sport_id: sport.id,
      status: 'published',
...
describe.runIf(enabled)('POST /api/articles/generate', () => {
  it('returns 401 without x-user-id header', async () => {
    const res = await POST('/api/articles/generate', {
  ...
  it('returns 409 when article already exists for this event', async () => {
    ...
    const res = await POST('/api/articles/generate', { event_id: event.id }, await authHeaders())
